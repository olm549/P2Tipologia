...
ml.heart <- lm(output~age+sex+slp+thalachh+restecg+cp, data = heart_ori,family = binomial)
...
r
```
El resultado del modelo es de 0.8326, por lo que la habilidad del modelo para predecir
...
Procedemos a instalar los packages necesarios:
...
summary(mo)
```
Obtenemos la siguiente información del arbol de decisión:

El algoritmo ha obtenido 8 reglas de clasificación con un error del 8.4% y un acierto del 92%.

Analizaremos los mejores resultados:

class Bajo riesgo  [0.953](rule 1): Hombre con tipo de dolor en el pecho asintomático y con número de vasos principales en el corazón superior a cero tiene un 95% de no padecer enfermedad cardíaca. 

class Alto riesgo  [0.955](Rule 6): Persona con edad de 59 años, con la presión arterial en resposo <= 155 con número de vasos principales en el corazón <= 0 y con los resultados en la prueba de esfuerzo cardíaco 0 o 2 tiene un 95% de probabilidad de padecer enfermedad cardíaca. 

Las variables más influyentes en la clasificación son: 87.13%	cp, 81.19%	thall, 53.47%	caa, 51.49%	sex, 48.51%	age, 31.68%	trtbps, 19.80%	chol, 19.80%	slp, 8.42%	exng
...
El estudio de los datos nos muestra que 38 + 38 = 76 has sido clasificados correctamente y 11 + 14 = 25 no. Tenemos un 76% de aciertos, un porcentaje de acierto inferior al conjunto de entrenamiento.
...
plot(2:10,resultados[2:10],type="o",col="blue",pch=0,xlab="Numbero de clusters",ylab="tot.withinss")
```
...
fit_ch$bestk
fit_asw$bestk
plot(1:10,fit_ch$crit, type = "o", col="blue", pch=0, xlab="Número de clústers", ylab="Criterio Silueta Media")
...
exactitud = 218/(218+85) = 0,71947, con lo que el modelo para k=2 tienen una exactitud del 72%.

******
## Conclusión
******

Inicialmente hemos realizado un proceso de limpieza y preprocesado de los datos del dataset que estamos estudiando, el cual contiene datos reales sobre pruebas médicas para el diagnóstico de enfermedades cardíacas. En este proceso hemos cambiado los nombres a las variables y hemos estudiado los valores extresmos(Outliers).
